...
public class StockLossServiceTest {
    ...
    @Test
    public void testLossServiceCountCheck(){
        //Arrange
        RoastingBatchEntity batch = new RoastingBatchEntity("TestCountry",
                1,
                330600,
        ...
        //check if saved in db then act
        Optional<RoastingBatchEntity> b = roastingBatchRepo.findOneByOriginCountry("TestCountry");
        if(b.isPresent()){
            // Act
            stockLossService.updateLoss(b.get().getBatchId(),
                    b.get().getInputWeight(),
                    b.get().getOutputWeight());
        }

        Optional<RoastingBatchEntity> bUpdated = roastingBatchRepo.findOneByOriginCountry("TestCountry");
        assertThat(bUpdated).isPresent();
        assertThat(bUpdated.get().getOriginCountry()).isEqualTo(batch.getOriginCountry());
        assertThat(bUpdated.get().getLossPercentage()).isEqualTo(13);
